...
An additional bonus is given as a portion of their total costs for the quarter (up to a limit of 65% of their costs). It is calculated as per their loyalty, performance, and efficiency.
...
3. Efficiency &rarr; if 'SLOW' or 'HIGHCOST' are in their analysis, then there is no bonus given. If no inefficiencies, 10% bonus. If there is only high traffic, 5% bonus.
...
## Scoring
Implementations are based on functionality and speed. You are graded with a 50% weightage toward successful implementations and a 50% weightage towards speedup over a naive staff implementation.
