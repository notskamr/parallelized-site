...
## Getting Started
Pleae clone this repository locally: https://github.com/parallelized/parallelized-starter
...
7 main functions. `calculate_overall_customer_satisfaction`, `calculate_driver_effiency`, `analyse_driver_efficiency`, `analyse_driver_performance`, `calculate_payout`, `calculate_bonus` and `insert_analysis_row`
Along the way, you can implement any required helper functions as needed (some have been provided already in `database.py`)

## Task 1: Implement 'overall_customer_satisfaction'
`calculate_overall_custom_satisfaction` should get the average ratings overall rides.
...
1. They have a poor average cost/km with a specific vehicle (higher than Rs. 3 for 2 wheelers, higher than Rs. 6 for 3 wheelers, higher than Rs. 15 for 4 wheelers)
2. They are very slow in terms of speed (below 20km/h)
3. They have faced poor traffic
You should find which one of these three applies to each driver, and add to a comma-seperated string. This string will eventually be stored. 

## Task 4: Implement 'analyse_driver_performance'
The average ratings of a driver should be calculated. This will eventually be stored.

## Task 5: Implement 'calculate_payout'
Will be released soon.

## Task 6: Implement 'insert_analysis_row'
Will be released soon.
